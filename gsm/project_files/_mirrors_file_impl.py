...
class MirrorsFile_Mirror(GsmFileStruct):
    ...
    def validate(self):
        ...
        assert self.id != "", f"invalid {MIRRORS_FILE_PATH}, mirror id is empty, mirror: '{self.remote}'"
        
        # TODO: checar se o nome do arquivo do path tem as extenções corretas

        # check if the hash is a valid hash digest
        assert is_valid_hash(self.hash), f"invalid {MIRRORS_FILE_PATH}, mirror hash is not valid, mirror: '{self.remote}', invalid hash: '{self.hash}'"
